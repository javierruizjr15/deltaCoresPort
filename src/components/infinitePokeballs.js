export const Pokeballs = () => {
    ...
    const FRAGMENT_SHADER = `
        ...
        const vec3 ambientLight = 0.89 * vec3(1.0, 1.0, 1.0);
        ...
        const float stopThreshold = 0.0001;
        ...
        vec3 _p = p;
        float _rand = max(rand(floor(_p).xz), .2);

        p.x = mod(p.x, 1.) - .5;
        p.z = mod(p.z, 1.) - .5;

        vec3 spherepos = p + vec3(0., -.05, 0.);
        spherepos.y -= abs(sin(u_time * 5. + _rand * 1000.) * .2 * (_rand+.2));
        spherepos = (vec4(spherepos, 1.) * rotationMatrix(vec3(1., 0., 0.5), u_time * _rand * 2.)).xyz;

        mat4 cyl_rot = rotationMatrix(vec3(3., 0., 1.), 1.5708);
        float button_cyl = sdCappedCylinder((vec4(spherepos, 1.) * cyl_rot).xyz + vec3(.0, -0.1, 0.0), vec2(.05, .07));

        world = sdSphere(spherepos, .15);
        world = min(world, _p.y+.1);

        next_object = sdSphere(spherepos, .155);
        next_object = max(-sdBox(spherepos + vec3(0., .09, 0.), vec3(.5, .1, .5)), next_object);
        next_object = max(-button_cyl, next_object);
        world = min(world, next_object);
        ...
        next_object = button_cyl + .02;
        world = min(world, next_object);

        return world;
        }
        float world_sdf(in vec3 p, inout float object_id) {
        float world = 10.;
        ...
        object_id = 0.;
        world = sdSphere(spherepos, .15);
        // float next_object = p.y+.1;
        float next_object = _p.y+.1;
        if(world > p.y+.1) {
        object_id = 1.;
        world = next_object;
        }
        ...
        // Raymarching.
        float rayMarching( vec3 origin, vec3 dir, float start, float end, inout float field, inout float object_id ) {

        float sceneDist = 1e4;
        float rayDepth = start;
        for ( int i = 0; i < maxIterations; i++ ) {
        sceneDist = world_sdf( origin + dir * rayDepth, object_id ); // Distance from the point along the ray to the nearest surface point in the scene.
        ...
        for (int i=0; i<maxIterationsShad; i++){
            ...
            dist += min(h, stepDist); // The best of both worlds... I think. 
        ...
        * */
        ...
        // Lighting.
        vec3 lighting( vec3 sp, vec3 camPos, int reflectionPass, float dist, float field, float object_id, vec3 rd) {
        ...
        vec3 lp = vec3(.5, .5, -.5) + movement;
        // ld - Light direction.
        vec3 ld = lp-sp;
        // lcolor - Light color.
        vec3 lcolor = vec3(1.) * .5;
        ...
        float ambient = .8; //The object's ambient property.
        ...
        float shadow = softShadow(sp, lp, .02, 2.);
        sceneColor *= shadow + .4;
        ...
        vec3 lookAt = vec3(sin(u_mouse.x * PI), 0., cos(u_mouse.x * PI));  // This is the point you look towards, or at, if you prefer.
        vec3 camera_position = vec3(0., .3 + max(u_mouse.y, -.3), -1.0); // This is the point you look from, or camera you look at the scene through. Whichever way you wish to look at it.
        ...
        float field = 0.;
        float object_id = 0.;
        float dist = rayMarching(ro, rd, clipNear, clipFar, field, object_id );
        if ( dist >= clipFar ) {
        ...
        vec3 sceneColor = lighting( sp, camera_position, 0, dist, field, object_id, rd);
    ...
    document.addEventListener('pointermove', (e)=> {
        ...
        if(uniforms.u_mouse.value.x < -0.7) uniforms.u_mouse.value.x = -0.7;
        if(uniforms.u_mouse.value.x > 0.7) uniforms.u_mouse.value.x = 0.7;
